...
def signup(request):
    ...
    try:
        user = CustomUser.objects.create(username=username, email=email, phone=phone, password=password, role=role)
    ...
    try:
        ...
        if not check_password(password, user.password):
            ...
            if user.role == 'admin':
                ...
                feedback_serializer = AdminFeedbackSerializer(feedbacks, many=True)
                ...
                feedbacks = Feedback.objects.all()
                feedback_serializer = StaffFeedbackSerializer(feedbacks, many=True)
                return Response({
                    'message': 'Login successful',
                    'feedbacks': feedback_serializer.data
                }, status=status.HTTP_200_OK)
